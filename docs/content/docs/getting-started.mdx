---
title: Getting Started
description: เริ่มติดตั้ง สร้างโปรเจกต์ และ sync component manifest ด้วย WebX CLI
---

WebX CLI ออกแบบให้เรียกผ่าน `bunx webx` ได้โดยตรง มีคำสั่งหลักสำหรับสร้างโปรเจกต์จาก GitHub template และ sync component list เข้า `webx.json`

## Create Project

```bash
bunx webx create my-app
```

คำสั่งนี้จะ:

- clone WebX template จาก GitHub
- ลบ `.git` ของ template ออกเพื่อให้เริ่ม repo ใหม่ได้สะอาด
- หา `package.json` ในโปรเจกต์และโฟลเดอร์ย่อย
- รัน dependency install ให้อัตโนมัติด้วย `bun` ถ้ามี หรือ fallback เป็น `npm`

## Create From Another Repository

```bash
bunx webx create my-app --repo https://github.com/owner/repo.git
```

ระบุ branch:

```bash
bunx webx create my-app --repo https://github.com/owner/repo.git --branch main
```

ข้าม install:

```bash
bunx webx create my-app --no-install
```

เก็บ `.git` จาก template ไว้:

```bash
bunx webx create my-app --keep-git
```

กำหนด package manager:

```bash
bunx webx create my-app --pm bun
bunx webx create my-app --pm npm
```

## Generate Component Manifest

หลังเพิ่มหรือลบ component ใน `web/components` ให้รัน:

```bash
bunx webx generate
```

คำสั่งนี้จะ scan folder ใน `web/components` ที่มีทั้ง `app.js` และ `template.html` แล้วแก้ `web/webx.json` อัตโนมัติ

ตัวอย่างผลลัพธ์:

```txt
Components
  counter (component)
  home (main)
```

และจะเขียน config:

```json
{
    "main": "home",
    "components": [
        "counter"
    ]
}
```

## Choose Main Component

ถ้าต้องการกำหนด main เอง:

```bash
bunx webx generate --main home
```

ถ้าไม่กำหนด CLI จะใช้ค่าเดิมใน `webx.json` ก่อน ถ้าไม่มีจะเลือก `home` ถ้าพบ component ชื่อนี้ ไม่อย่างนั้นจะเลือก component แรกตามลำดับชื่อ

## Dry Run

ดู component และ manifest ที่จะเขียนโดยไม่แก้ไฟล์:

```bash
bunx webx generate --dry-run
```

## Custom Paths

ถ้าโครงสร้างโปรเจกต์ไม่ใช้ `web` เป็นโฟลเดอร์หลัก:

```bash
bunx webx generate --web apps/nui
```

ถ้า component อยู่คนละ path ภายใน web dir:

```bash
bunx webx generate --components ui/components
```

## Minimal HTML

หน้า HTML ต้องมี root element สำหรับ render และ import runtime หลักของ WebX ผ่าน ES module

```html
<div id="app"></div>

<script type="module" src="./wbx/app.js"></script>
```

ในโปรเจกต์ตัวอย่างใช้ import map เพื่อให้ runtime import path แบบ alias ได้

```html
<script type="importmap">
{
  "imports": {
    "@components/": "./components/",
    "@directives/": "./directives/",
    "@stores/": "./stores/",
    "@wbx/": "./wbx/",
    "zod": "./wbx/lib/zod.min.js"
  }
}
</script>
```

## Configure `webx.json`

`webx.json` บอก runtime ว่า component ไหนเป็นหน้าหลัก และ component ใดต้อง register เพื่อเรียกใช้ใน template

```json
{
  "main": "home",
  "components": ["counter"]
}
```

- `main` คือชื่อ folder component หลักใน `web/components`
- `components` คือรายชื่อ component ลูกที่ต้องการ register
- runtime จะเพิ่ม `main` เข้าไปตอน validate เพื่อโหลด component หลักด้วย

## First Component

สร้าง folder component โดยใช้ชื่อ lower-case เช่น `web/components/home`

```txt
web/components/home/
  app.js
  template.html
  style.css
```

`app.js`

```js
export default {
  template: 'Home',
  data: {
    title: 'Hello WebX'
  },
  method: {
    rename() {
      this.data.title = 'Updated title'
    }
  }
}
```

`template.html`

```html
<template>
  <main class="home">
    <h1>{{ title }}</h1>
    <button @click="rename">Rename</button>
  </main>
</template>
```

`style.css`

```css
.home {
  max-width: 720px;
  margin: 32px auto;
}
```
